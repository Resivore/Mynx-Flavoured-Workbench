...
public class ConcretePowderWall extends FallingWall {
    ...
    public void onLand(Level world, BlockPos pos, BlockState fallingBlockState, BlockState currentStateInPos, FallingBlockEntity fallingBlockEntity) {
        if (ConcretePowderSemantics.shouldHarden(world, pos, currentStateInPos)) {
            world.setBlock(pos, ConcretePowderSemantics.hardenedState(fallingBlockEntity.getBlockState(), this.hardenedBlock.getBlock(ModBlocks.BlockType.WALL)), 3);
        }
    ...
    public BlockState updateShape(BlockState state, LevelReader world, ScheduledTickAccess tickAccess, BlockPos pos, Direction direction, BlockPos neighborPos, BlockState neighborState, RandomSource random) {
        return ConcretePowderSemantics.shouldHarden(world, pos, state) ? ConcretePowderSemantics.hardenedState(state, this.hardenedBlock.getBlock(ModBlocks.BlockType.WALL)) : super.updateShape(state, world, tickAccess, pos, direction, neighborPos, neighborState, random);
    }

    public void tick(BlockState state, ServerLevel world, BlockPos pos, RandomSource random) {
        if (ConcretePowderSemantics.shouldHarden(world, pos, state)) { world.setBlock(pos, ConcretePowderSemantics.hardenedState(state, this.hardenedBlock.getBlock(ModBlocks.BlockType.WALL)), 3); return; }
