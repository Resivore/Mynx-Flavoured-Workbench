param(
    [string]$PristineJar = (Join-Path (Split-Path -Parent $PSScriptRoot) 'build-inputs\moreslabsstairsandwalls-4.2.0.jar')
)
...
$outputRoot = Join-Path $projectRoot 'common\src\main\generated'
$staticRoots = @(
    (Join-Path $projectRoot 'common\src\main\resources'),
    (Join-Path $projectRoot 'fabric\src\main\resources')
)
$resolvedOutput = [System.IO.Path]::GetFullPath($outputRoot)
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $projectRoot 'common\src\main\generated'))
...
$actualPristineSha256 = (Get-FileHash -LiteralPath $PristineJar -Algorithm SHA256).Hash
