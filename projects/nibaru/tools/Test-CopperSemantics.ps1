...
$api = Get-Content -Raw (Join-Path $root 'common/src/main/java/games/twinhead/moreslabsstairsandwalls/api/material/NibaruMaterialProfiles.java')
$semantics = Get-Content -Raw (Join-Path $root 'common/src/main/java/games/twinhead/moreslabsstairsandwalls/block/oxidizable/CopperSemantics.java')
$oxidizable = Get-Content -Raw (Join-Path $root 'common/src/main/java/games/twinhead/moreslabsstairsandwalls/block/oxidizable/CustomOxidizable.java')
$waxed = Get-Content -Raw (Join-Path $root 'common/src/main/java/games/twinhead/moreslabsstairsandwalls/block/oxidizable/CustomWaxedCopper.java')
$classes = Get-ChildItem (Join-Path $root 'common/src/main/java/games/twinhead/moreslabsstairsandwalls/block/oxidizable') -Filter '*.java' | ForEach-Object { Get-Content -Raw $_.FullName }
$checks = [ordered]@{
