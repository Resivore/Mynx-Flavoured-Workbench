...
$javaRoot = Join-Path $root 'common\src\main\java\games\twinhead\moreslabsstairsandwalls'
$generatedRoot = Join-Path $root 'common\src\main\generated'
...
$client = Get-Content -Raw -LiteralPath (Join-Path $root 'fabric\src\main\java\games\twinhead\moreslabsstairsandwalls\fabric\MoreSlabsStairsAndWallsFabricClient.java')
...
$effectiveResourceRoots = @(
    $generatedRoot,
    (Join-Path $root 'common\src\main\resources'),
    (Join-Path $root 'fabric\src\main\resources')
)
...
function Get-ResourceAggregate([string]$Pattern) {
    $resourceRoot = Join-Path $root 'common\src\main\resources'
