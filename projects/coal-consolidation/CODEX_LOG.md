...
## 2026-08-31T01:00:05Z — Build Coal Consolidation Canary 3
- Revision: 6
- Source checkpoint: `d2ad841f6a8edfbc765ec681d803b193e5d1f7ad`
- Changes: Treated the outside-Workbench observation that furnace logs produced coal while smoker logs produced charcoal as defect evidence only because exact artifact/runtime identity is unproven. Confirmed the packaged smoker JSON already returns coal, identified C2's furnace-only post-reload enforcement gap, and extended the same narrow RecipeManager transformation to require and rebuild exactly `minecraft:charcoal` as SmeltingRecipe and `smoking:charcoal` as SmokingRecipe. Both holders retain their IDs, input objects, category/group/notification metadata, experience, and cooking times while only the result becomes one coal; either missing, duplicate, or wrong-type target fails closed. Added a later competing smoker-pack effective-reload regression and focused negative coverage without changing unrelated recipes or existing data contracts.
- Build/static: Temurin Java 25, Gradle 9.5.1, and Loom 1.17.20 completed `clean test build --no-daemon`; all 19 tests in four suites passed with zero failures, errors, or skips, including both furnace and smoker competing-pack reload regressions and the seven-consumer, cleanup-conversion, tag, registry, Creative, and recipe-viewer contracts. Built and retained C3 JARs are byte-identical. This is static evidence only.
- Runtime: No Minecraft launch, Workbench test-slot allocation, deployment, Test Instance Manager mutation, runtime-profile access, dedicated 26.2 Workbench mutation, protected 26.1.2 gameplay-instance access, or canonical gameplay result occurred. The outside observation is not recorded as an official pass or fail; C3 remains runtime untested.
- Artifact: Current and unaccepted `0.1.2-canary3` is `coal-consolidation-0.1.2-canary3.jar`, 11,711 bytes, SHA-256 `f1d36dfad276927d527cc911d92f47ee979ef7600479f950c76ed25dd18eb871`, from release source `d2ad841f6a8edfbc765ec681d803b193e5d1f7ad`; C1 and C2 were not overwritten, and no accepted or rollback release exists.
- Result: `ACTIVE` — C3 is `STATIC_PASS / NOT_DEPLOYED / RUNTIME_UNTESTED`, occupies neither test slot, has no formal blocker, and changes only the two owned effective charcoal-cooking holders while retaining all prior project contracts.
- Next state: Under separate authorization, allocate only exact retained C3 through the canonical manager, verify the intended Matcha 1.12 stack and readiness, then run and record the full matrix with explicit furnace and smoker coverage before any acceptance decision.

## 2026-08-31T04:36:25Z — Accept exact Coal Consolidation Canary 3 in Stack v6
- Revision: 7
- Source checkpoint: `12d96636943b4ba405c0d96916bbdc74e1434629`
- Changes: Recorded the user's authorization and aggregate exact-candidate runtime PASS; promoted unchanged C3 from current/unaccepted to current/accepted with no separate rollback; and narrowed the retained testing record to the accepted identity and future regression procedure. The atomic user-passed MOD batch advanced the accepted baseline from Stack v3 to Stack v6 without changing either occupied test slot.
- Build/static: No rebuild or new static analysis was performed. Preserved evidence remains Java 25, Gradle 9.5.1, Loom 1.17.20, 19/19 passing focused tests, a successful C3 JAR build, and byte identity between the built and retained artifact.
- Runtime: The user reported an aggregate PASS for exact C3 and authorized acceptance; no individual checklist result is inferred. The Test Instance Manager atomically installed the exact artifact and subsequently verified physical Stack v6 at runtime-state revision 50 with both existing slots preserved. Manager verification is deployment evidence, not an additional gameplay observation.
- Artifact: Current and accepted `0.1.2-canary3` is unchanged `coal-consolidation-0.1.2-canary3.jar`, 11,711 bytes, SHA-256 `f1d36dfad276927d527cc911d92f47ee979ef7600479f950c76ed25dd18eb871`, from release source `d2ad841f6a8edfbc765ec681d803b193e5d1f7ad`; no rollback release is designated, and no candidate binary was rebuilt or altered.
- Result: ACCEPTED — exact C3 is `STATIC_PASS / READY_TO_TEST_VERIFIED / RUNTIME_PASS`, current and accepted in verified Stack v6, with no separate rollback and no blocker.
- Next state: Preserve this exact accepted identity. Any material successor requires a distinct version and artifact, independent static and runtime evidence, and a separately authorized accepted-stack transition; do not reuse failed C1 or unaccepted C2 as rollback.
