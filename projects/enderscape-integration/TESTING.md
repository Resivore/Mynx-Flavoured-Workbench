# Testing — Enderscape Integration Canary 5

Test only the exact release below with the unchanged upstream Enderscape input.

| Field | Value |
|---|---|
| Companion artifact | `enderscape-integration-0.1.0-canary5.jar` |
| Version | `0.1.0-canary5` |
| Companion SHA-256 | `fb1b5d94d434c76d6350b495294a72904a4e1d42611faef8912fd03ad30db560` |
| Built at | `2026-09-22T04:14:49.3068769Z` |
| Implementation checkpoint | `d3836552f464612ef5c330b9ca9d27e8056fda0c` |
...
Canary 5 supersedes untested Canary 4 after static inspection corrected the
canonical-before-BGE injector order and limited the exact-Iris gate to the
optional shader callback. Canary 5 has controlled build/static evidence only.
It is `RUNTIME_UNTESTED`.
...
1. Start a disposable Minecraft 26.2 Fabric instance with the exact upstream
   Enderscape jar, Matcha Flavoured Data, and this exact Canary 5 companion. Confirm server/world startup
...
## 5. Optional Canary 5 Veiled Leaves shader-wind matrix
...
6. Remove Iris and all shaders for the no-Iris control. Start, join, reload, leave/rejoin, and restart once. Recheck the common-mixin paths explicitly: blocked recipes, suppressed advancements and enchantment/tag acquisition, native food healing, Chorus Cake Roll healing, and the retained C2/C3 recipes must all work normally with no Iris requirement or linkage error. Ordinary Veiled Leaves behavior must remain unchanged. If a different Iris build is otherwise compatible with this Minecraft instance, repeat the common-mixin checks once with that non-matching version and shaders disabled; only the exact-version shader callback may decline to load.
...
1. Run `/reload`, disconnect/reconnect, and reopen Creative Search and JEI.
   ...
   variant leak.
2. Repeat the retained C2 Rubblemite, Void Campfire, and Veiled Leaves checks plus every retained C3 JEI/localization and C5 shader-wind check after reload. Preserve logs and the exact release identifiers above with every observation.
...
No Minecraft, Iris, Complementary, BGE, JEI, or gameplay runtime observation has been supplied for C1-C5 yet.
