...
/** Registers C5's always-enabled, narrowly generated Enderscape data overlay. */
public final class EnderscapeIntegration implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        boolean registered = ResourceLoader.registerBuiltinPack(
                BUILTIN_PACK_ID,
                container,
                Component.literal("Enderscape Integration C5"),
                PackActivationType.ALWAYS_ENABLED);
        if (!registered) {
            throw new IllegalStateException("Could not register required Enderscape Integration data overlay");
        }
        LOGGER.info("Registered Enderscape Integration C5 overlay; exact upstream input SHA-256={}",
