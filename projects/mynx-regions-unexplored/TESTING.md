# Mynx Regions Unexplored — Canary 1 runtime procedure

Current candidate: `0.1.0+26.2-canary1`, `mynx-regions-unexplored-private-0.1.0+26.2-canary1.jar`. The canonical manifest records its exact SHA-256 and implementation checkpoint. It is ACTIVE, NOT_DEPLOYED, and RUNTIME_UNTESTED. Build and package checks are not Minecraft observations.
...
1. Confirm exactly these 16 items appear in Natural Blocks and work with `/give`: Dropleaf, Barley, Windswept Grass, Clover, Stone Bud, Mycotoxic Daisy, Hyssop, Blue/Pink/Purple/Red/Yellow Lupine, Cattail, Duckweed, Tassel and Meadow Sage. `dropleaf_plant` and all seven potted blocks must have no separate item. No other RU plants, world generation, trades or initial-acquisition path should exist.
2. Place, outline, break and support-test every plant in Survival. Verify correct names, sounds, collision, offsets, replaceability, drops and support loss. Hyssop and each Lupine use their compact reference shape. Mycotoxic Daisy is a static two-block plant, emits light level 4, grows on dirt-like blocks, nylium and soul soil, and applies no toxicity.
3. Stone Bud places only on the dedicated stone-support tag, not ordinary dirt/farmland; it has grass tint in world and item form, no bonemeal growth, one self drop with shears, and otherwise the vanilla short-grass wheat-seed chance including Fortune. It is flammable; Mycotoxic Daisy is deliberately not. Do not substitute or expose `grass_sprouts`.
4. Barley and Windswept Grass are static two-block plants. Breaking either half must clean the pair without duplicate drops. Barley drops one Barley. Windswept Grass drops two plants with shears and otherwise uses the tall-grass seed chance/Fortune behavior.
5. Clover must face the placer, stack from amount 1 through 4, drop its exact amount, fill through repeat placement/bonemeal, and eject one extra item when bonemealed at 4. It is not treated as a flower for bee or flower tags.

## Growth, water and renewal

1. Place Dropleaf beneath valid support with room below. Confirm the luminous head is age-based, grows downward randomly, creates non-random-ticking body segments, remains climbable, emits light level 14 only at the head, and uses 1–2 new blocks per bonemeal. Break head/body/support at several lengths and verify only one Dropleaf item is obtainable—never a body item or duplicate loot.
...
4. Test every item directly and by hopper against a composter over repeated trials. Expected chances: Dropleaf/Duckweed 15%; Clover 20%; Stone Bud/Mycotoxic Daisy 30%; six small flowers 40%; Windswept Grass 50%; Barley/Cattail/Tassel/Meadow Sage 60%.
5. Pot and retrieve exactly Hyssop, five Lupines and Mycotoxic Daisy. Each potted block must return its flower plus the pot through normal interaction, while block loot itself is only the flower pot. Potted Mycotoxic Daisy emits light level 8. Meadow Sage must not enter a pot.
...
2. With Complementary foliage waving enabled, compare Stone Bud, Clover and the six small flowers beside short grass; Barley, Windswept Grass, Mycotoxic Daisy, Cattail, Tassel and Meadow Sage beside tall grass; Duckweed beside a lily pad; and Dropleaf head/body beside weeping vines. Lower/upper tall halves must inherit their matching active-pack classifications. Pots must never inherit plant wind.
