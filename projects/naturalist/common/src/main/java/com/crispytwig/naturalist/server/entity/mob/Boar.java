...
public class Boar extends NaturalistAnimal implements NeutralMob, DataDrivenVariantAnimal {
    //region Data
    private static final java.util.function.Predicate<ItemStack> FOOD_ITEMS = stack -> stack.is(NaturalistTags.ItemTags.BOAR_FOOD_ITEMS);
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Mob.createMobAttributes().add(Attributes.TEMPT_RANGE, 10.0D).add(Attributes.MAX_HEALTH, 24.0).add(Attributes.MOVEMENT_SPEED, 0.25).add(Attributes.ATTACK_DAMAGE, 4.0D).add(Attributes.ATTACK_KNOCKBACK, 1.0D).add(Attributes.KNOCKBACK_RESISTANCE, 0.5D).add(Attributes.FOLLOW_RANGE, 16.0D);
    }
    ...
    public void thunderHit(@NotNull ServerLevel level, @NotNull LightningBolt lightning) {
        ...
        if (level.getDifficulty() != Difficulty.PEACEFUL) {
            ...
            assert zoglin != null;
            zoglin.absSnapTo(this.getX(), this.getY(), this.getZ(), this.getYRot(), this.getXRot());
