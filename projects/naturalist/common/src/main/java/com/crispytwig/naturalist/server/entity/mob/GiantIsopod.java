...
public class GiantIsopod extends Animal implements HidingAnimal, VariantBucketable {
    ...
    private static final java.util.function.Predicate<ItemStack> FOOD_ITEMS = stack -> stack.is(NaturalistTags.ItemTags.GIANT_ISOPOD_FOOD);
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Mob.createMobAttributes().add(Attributes.TEMPT_RANGE, 10.0D).add(Attributes.MAX_HEALTH, 12.0D).add(Attributes.MOVEMENT_SPEED, 0.4D);
    }
    ...
    private boolean computeCanHide() {
        ...
        return !this.level().getEntitiesOfClass(Player.class, this.getBoundingBox().inflate(3.0D, 2.0D, 3.0D),
                player -> EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(player)
                        && BehaviorCompat.matchesNonCombatRange(this, player, 3.0D)).isEmpty();
    }
