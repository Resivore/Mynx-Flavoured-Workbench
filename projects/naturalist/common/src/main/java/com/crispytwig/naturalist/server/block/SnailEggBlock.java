...
public class SnailEggBlock extends Block {
    ...
    @Override
    public @NotNull BlockState updateShape(@NotNull BlockState state, @NotNull LevelReader level,
    ...
    private void spawnBabySnails(ServerLevel level, @NotNull BlockPos pos, RandomSource random) {
        ...
        for(int j = 1; j <= i; ++j) {
            ...
            if (snail != null) {
                ...
                snail.snapTo(d, pos.getY(), (double)pos.getZ() + this.getRandomSnailPositionOffset(random),
                        (float)random.nextInt(1, 361), 0.0F);
