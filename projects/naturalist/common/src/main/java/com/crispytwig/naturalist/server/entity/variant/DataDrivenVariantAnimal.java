...
public interface DataDrivenVariantAnimal {
    String VARIANT_TAG = NaturalistEntityPersistence.VARIANT_TAG;
    ...
    default void saveVariant(ValueOutput output) {
        NaturalistEntityPersistence.saveVariant(output, this.getVariantLocation());
    }
    ...
    default void loadVariant(ValueInput input) {
        input.getString("id")
                .flatMap(LegacyVariantRemap::variantForLegacyEntityId)
                .or(() -> NaturalistEntityPersistence.readVariant(input, this.getLegacyVariantNames()))
                .ifPresent(location -> this.setVariantString(location.toString()));
    }
