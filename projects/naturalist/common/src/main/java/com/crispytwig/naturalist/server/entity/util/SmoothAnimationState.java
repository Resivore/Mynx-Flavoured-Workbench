...
public class SmoothAnimationState extends AnimationState {
    ...
    private long animationAccumulatedTime;
    ...
    public void start(int tickCount) {
        ...
        this.animationLastTime = tickCount * 1000L / 20L;
        this.animationAccumulatedTime = 0L;
    }

    @Override
    public void stop() {
        super.stop();
        this.animationLastTime = 0L;
    }

    /**
     * Retains the pre-26.2 {@link AnimationState} time accumulator semantics.
     * Naturalist changes animation speed while an animation is running, so
     * deriving time directly from the start tick would introduce visible jumps.
     */
    public void updateTime(float ageInTicks, float speed) {
        ...
        long currentTime = Mth.lfloor(ageInTicks * 1000.0F / 20.0F);
        this.animationAccumulatedTime += (long) ((currentTime - this.animationLastTime) * speed);
        this.animationLastTime = currentTime;
    }

    public long getAccumulatedTime() {
        return this.animationAccumulatedTime;
    }
