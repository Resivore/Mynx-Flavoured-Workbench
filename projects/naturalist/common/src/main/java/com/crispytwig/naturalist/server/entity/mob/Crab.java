...
public class Crab extends TamableAnimal implements HidingAnimal, FollowingPet, Catchable, DataDrivenVariantAnimal {
    ...
    private static final java.util.function.Predicate<ItemStack> FOOD_ITEMS = stack -> stack.is(NaturalistTags.ItemTags.CRAB_FOOD);
    ...
    public void saveToHandTag(@NotNull ItemStack stack) {
        ...
        if (!held.isEmpty()) {
            tag.store("HeldItem", ItemStack.CODEC,
                    this.level().registryAccess().createSerializationContext(NbtOps.INSTANCE), held);
        }
    ...
    public void loadFromHandTag(@NotNull CompoundTag tag) {
        ...
        tag.read("HeldItem", ItemStack.CODEC,
                this.level().registryAccess().createSerializationContext(NbtOps.INSTANCE)).ifPresent(held -> {
    ...
    private boolean thinkCanHide() {
        ...
        List<? extends Player> players = this.level().players().stream()
                .filter(player -> this.getBoundingBox().inflate(4.0D, 2.0D, 4.0D).contains(player.position()))
                .filter(EntitySelector.NO_CREATIVE_OR_SPECTATOR)
                .filter(player -> BehaviorCompat.matchesNonCombatRange(this, player, 4.0D))
                .toList();
