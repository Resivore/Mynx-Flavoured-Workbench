...
import com.crispytwig.naturalist.server.entity.util.BehaviorCompat;
...
public class Ostrich extends TamableAnimal implements EggLayingAnimal, HidingAnimal, FollowingPet, DyeableAnimal, PlayerRideableJumping, IKMount, NeutralMob, DataDrivenVariantAnimal {
    //region Data
    private static final Predicate<ItemStack> FOOD_ITEMS = stack -> stack.is(NaturalistTags.ItemTags.OSTRICH_FOOD_ITEMS);
    ...
    public void setSaddled(boolean saddled) {
        ItemStack saddleStack = this.getItemBySlot(EquipmentSlot.SADDLE);
        if (saddled && saddleStack.isEmpty()) {
            this.setItemSlot(EquipmentSlot.SADDLE, new ItemStack(Items.SADDLE));
        } else if (!saddled && !saddleStack.isEmpty()) {
    ...
    public void onEquipItem(EquipmentSlot slot, ItemStack oldStack, ItemStack newStack) {
        ...
        if (slot == EquipmentSlot.SADDLE) {
            this.entityData.set(SADDLED, !newStack.isEmpty());
        }
    }
    ...
    public void addAdditionalSaveData(@NotNull ValueOutput compound) {
        ...
        compound.store("OwnedEggs", Codec.LONG.listOf(), this.ownedEggs.stream().map(BlockPos::asLong).toList());
    ...
    public void readAdditionalSaveData(@NotNull ValueInput compound) {
        ...
        this.setSaddled(compound.getBooleanOr("Saddled", false)
                || this.hasItemInSlot(EquipmentSlot.SADDLE));
        this.setHasEgg(compound.getBooleanOr("HasEgg", false));
        this.ownedEggs.clear();
        for (long packed : compound.read("OwnedEggs", Codec.LONG.listOf()).orElse(List.of())) {
    ...
    private boolean thinkCanHide() {
        ...
        if (this.level() instanceof ServerLevel serverLevel) {
            players = serverLevel.getNearbyPlayers(TargetingConditions.forNonCombat().range(16.0D)
            ...
            players = this.level().getEntitiesOfClass(Player.class, this.getBoundingBox().inflate(16.0D, 8.0D, 16.0D),
                    livingEntity -> BehaviorCompat.matchesNonCombatRange(this, livingEntity, 16.0D)
    ...
    public @NotNull InteractionResult mobInteract(@NotNull Player player, @NotNull InteractionHand hand) {
        ...
        if (this.isTame() && this.isOwnedBy(player) && this.isSaddled() && stack.is(NaturalistTags.ItemTags.SHEARS)) {
            if (!this.level().isClientSide()) {
                ItemStack equippedSaddle = this.getItemBySlot(EquipmentSlot.SADDLE).copy();
                this.setSaddled(false);
                this.spawnAtLocation((ServerLevel)this.level(), equippedSaddle.isEmpty()
                        ? new ItemStack(Items.SADDLE) : equippedSaddle);
    ...
    public boolean canUseSlot(EquipmentSlot slot) {
        return slot == EquipmentSlot.SADDLE ? this.isSaddleable() : super.canUseSlot(slot);
    }

    public void equipSaddle(@NotNull ItemStack stack, @Nullable SoundSource source) {
        this.setItemSlot(EquipmentSlot.SADDLE, stack.copyWithCount(1));
        this.setGuaranteedDrop(EquipmentSlot.SADDLE);
    ...
    private void updateEggAnger() {
        ...
        if (engaged) {
            // 2.0.3 paused this countdown while the ostrich actively defended its target.
