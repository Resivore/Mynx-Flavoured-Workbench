...
public class Tortoise extends TamableAnimal implements HidingAnimal, EggLayingAnimal, FollowingPet, DataDrivenVariantAnimal {
    ...
    private static final Predicate<ItemStack> TEMPT_ITEMS = stack -> stack.is(NaturalistTags.ItemTags.TORTOISE_TEMPT_ITEMS);
    ...
    public static AttributeSupplier.@NotNull Builder createAttributes() {
        return Mob.createMobAttributes().add(Attributes.TEMPT_RANGE, 10.0D).add(Attributes.MOVEMENT_SPEED, 0.17f).add(Attributes.MAX_HEALTH, 30.0).add(Attributes.ATTACK_DAMAGE, 2.0).add(Attributes.KNOCKBACK_RESISTANCE, 0.6);
    }
    ...
    public AgeableMob getBreedOffspring(@NotNull ServerLevel serverLevel, @NotNull AgeableMob ageableMob) {
        ...
        if (ageableMob instanceof Tortoise tortoiseParent) {
            ...
            tortoise.setOwnerReference(this.random.nextBoolean()
                    ? tortoiseParent.getOwnerReference()
    ...
    public boolean canHide() {
        ...
        if (this.level() instanceof ServerLevel serverLevel) {
            players = serverLevel.getNearbyPlayers(TargetingConditions.forNonCombat().range(5.0D)
                            .selector((livingEntity, level) -> EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(livingEntity)
                                    && !livingEntity.isDiscrete() && !livingEntity.isHolding(TEMPT_ITEMS)),
                    this, this.getBoundingBox().inflate(5.0D, 3.0D, 5.0D));
        } else {
            players = this.level().getEntitiesOfClass(Player.class, this.getBoundingBox().inflate(5.0D, 3.0D, 5.0D),
                    livingEntity -> BehaviorCompat.matchesNonCombatRange(this, livingEntity, 5.0D)
