...
public class Giraffe extends TamableAnimal implements IKMount, DataDrivenVariantAnimal {
    //region Data
    private static final java.util.function.Predicate<ItemStack> FOOD_ITEMS = stack -> stack.is(NaturalistTags.ItemTags.GIRAFFE_FOOD_ITEMS);
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Mob.createMobAttributes().add(Attributes.TEMPT_RANGE, 10.0D).add(Attributes.MAX_HEALTH, 32.0D).add(Attributes.MOVEMENT_SPEED, 0.25F).add(Attributes.STEP_HEIGHT, 1.0D);
    }
