...
public class Rhino extends NaturalistAnimal implements DataDrivenVariantAnimal {
    ...
    static class RhinoChargeGoal extends Goal {
        ...
        protected void tryToHurt() {
            ...
            List<LivingEntity> nearbyEntities = level.getNearbyEntities(LivingEntity.class, TargetingConditions.forCombat(), this.mob, this.mob.getBoundingBox());
            if (!nearbyEntities.isEmpty()) {
                LivingEntity livingEntity = nearbyEntities.getFirst();
                if (!(livingEntity instanceof Rhino)) {
                    DamageSource source = livingEntity.damageSources().mobAttack(this.mob);
                    float damage = (float)this.mob.getAttributeValue(Attributes.ATTACK_DAMAGE);
                    boolean blocked = BehaviorCompat.wouldBlockDamage(livingEntity, source, damage);
                    ...
                    livingEntity.knockback(shieldBlockModifier * speed * 2.0D, this.chargeDirection.x(), this.chargeDirection.z(), source, damage);
