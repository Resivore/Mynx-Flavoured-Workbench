...
public class Hedgehog extends TamableAnimal implements DyeableAnimal, FollowingPet, HidingAnimal, Catchable, DataDrivenVariantAnimal {
    ...
    private static final java.util.function.Predicate<ItemStack> FOOD_ITEMS = stack -> stack.is(NaturalistTags.ItemTags.HEDGEHOG_FOOD_ITEMS);
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Mob.createMobAttributes().add(Attributes.TEMPT_RANGE, 10.0D).add(Attributes.MAX_HEALTH, 12.0).add(Attributes.MOVEMENT_SPEED, 0.2);
    }
    ...
    private boolean thinkCanHide() {
        ...
        if (this.level() instanceof ServerLevel serverLevel) {
            players = serverLevel.getNearbyPlayers(TargetingConditions.forNonCombat().range(6.0)
                    .selector((entity, level) -> EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(entity)),
                    this, this.getBoundingBox().inflate(6.0, 3.0, 6.0));
        } else {
            players = this.level().getEntitiesOfClass(Player.class, this.getBoundingBox().inflate(6.0, 3.0, 6.0),
                    player -> EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(player)
                            && BehaviorCompat.matchesNonCombatRange(this, player, 6.0D));
        }
            ...
            if (this.isRolling()) {
                ...
                if (this.returning) {
                    ...
                    for (LivingEntity target : this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(0.2), this::isSpikeTarget)) {
                        ...
                        DamageSource impactSource = this.damageSources().thrown(this, this.getOwner());
                        float impactDamage = 2.0F + this.getThrowEnchantmentLevel(Enchantments.THORNS);
                        if (target.hurtServer(serverLevel, impactSource, impactDamage)) {
                            int punch = this.getThrowEnchantmentLevel(Enchantments.PUNCH);
                            if (punch > 0) {
                                target.knockback(punch * 0.6, -motion.x, -motion.z, impactSource, impactDamage);
                            }
                            if (flaming) {
                            ...
                            serverLevel.sendParticles(new ItemParticleOption(ParticleTypes.ITEM, net.minecraft.world.item.ItemStackTemplate.fromNonEmptyStack(particleStack)), this.getX(), this.getY(0.5), this.getZ(), 8, 0.1, 0.1, 0.1, 0.05);
