...
public class Hippo extends TamableAnimal implements FollowingPet, DataDrivenVariantAnimal {
    //region Data
    private static final java.util.function.Predicate<ItemStack> FOOD_ITEMS = stack -> stack.is(Blocks.MELON.asItem());
        ...
        if (this.isFood(itemStack)) {
            ...
            if (age == 0 && this.canFallInLove()) {
                if (!this.level().isClientSide()) {
                    ...
                    ((ServerLevel)level()).sendParticles(new ItemParticleOption(ParticleTypes.ITEM, net.minecraft.world.item.ItemStackTemplate.fromNonEmptyStack(new ItemStack(Items.MELON_SLICE))), this.getX() + Math.cos(yRot), this.getY() + 0.6, this.getZ() + Math.sin(yRot), 100, this.getBbWidth() / 4.0F, this.getBbHeight() / 4.0F, this.getBbWidth() / 4.0F, 0.05D);
