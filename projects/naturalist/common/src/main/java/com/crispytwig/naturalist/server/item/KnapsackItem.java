...
public class KnapsackItem extends Item {
    ...
    public @NotNull InteractionResult interactLivingEntity(@NotNull ItemStack stack, @NotNull Player player, @NotNull LivingEntity target, @NotNull InteractionHand hand) {
        ...
        try (ProblemReporter.ScopedCollector problems =
                     new ProblemReporter.ScopedCollector(mob.problemPath(), LOGGER)) {
            TagValueOutput output = TagValueOutput.createWithContext(problems, mob.registryAccess());
            mob.save(output);
    ...
    public @NotNull InteractionResult useOn(@NotNull UseOnContext context) {
        ...
        if (level instanceof ServerLevel serverLevel) {
            ...
            try (ProblemReporter.ScopedCollector problems = new ProblemReporter.ScopedCollector(LOGGER)) {
                ValueInput input = TagValueInput.create(problems, serverLevel.registryAccess(), tag);
                entity = EntityType.loadEntityRecursive(input, serverLevel, EntitySpawnReason.LOAD, e -> {
    ...
    public void appendHoverText(@NotNull ItemStack stack, @NotNull TooltipContext context,
        ...
        if (tag.contains("CustomName") && context.registries() != null) {
            ValueInput input = TagValueInput.create(ProblemReporter.DISCARDING, context.registries(), tag);
            label = input.read("CustomName", ComponentSerialization.CODEC).orElse(null);
        }
        if (label == null) {
            label = tag.getString("id").map(Identifier::tryParse)
