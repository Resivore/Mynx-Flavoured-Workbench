...
public final class VariantItemModels {
    ...
    public static Map<Identifier, NaturalistBucketItem> collectVariantItems() {
    ...
    public static List<Identifier> scanExtraModels(ResourceManager resourceManager) {
        return resourceManager.listResources("models/" + MODEL_FOLDER, path -> path.getPath().endsWith(".json")).keySet().stream()
                .map(file -> Identifier.fromNamespaceAndPath(file.getNamespace(),
                        file.getPath().substring("models/".length(), file.getPath().length() - ".json".length())))
                .toList();
    }

    public static Optional<Identifier> resolveModelId(NaturalistBucketItem item, ItemStack stack, @Nullable Level level) {
