...
public class Snake extends TamableClimbingAnimal implements SleepingAnimal, NeutralMob, FollowingPet, HuntingAnimal, DataDrivenVariantAnimal {
    ...
    private static final EntityDataAccessor<Long> ANGER_END_TIME = SynchedEntityData.defineId(Snake.class, EntityDataSerializers.LONG);
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(5, new NearestAttackableTargetGoal<>(this, Mob.class, 5, true, false, (livingEntity, level) -> this.canHunt() && (livingEntity.is(NaturalistTags.EntityTypes.SNAKE_HOSTILES) || (livingEntity instanceof Slime slime && slime.isTiny()))));
    ...
    public boolean hurtServer(@NotNull ServerLevel level, @NotNull DamageSource source, float amount) {
        if (!this.getMainHandItem().isEmpty()) {
            ItemEntity itemEntity = new ItemEntity(level, this.getX() + this.getLookAngle().x, this.getY() + 1.0D, this.getZ() + this.getLookAngle().z, this.getMainHandItem());
    ...
    public boolean canHoldItem(@NotNull ItemStack itemStack) {
        return this.getMainHandItem().isEmpty() && FOOD_ITEMS.test(itemStack);
    }
    ...
    private boolean canRattle() {
        ...
        if (this.level() instanceof ServerLevel serverLevel) {
            players = serverLevel.getNearbyPlayers(TargetingConditions.forNonCombat().range(4.0D), this,
                    this.getBoundingBox().inflate(4.0D, 2.0D, 4.0D));
        } else {
            players = this.level().getEntitiesOfClass(Player.class, this.getBoundingBox().inflate(4.0D, 2.0D, 4.0D),
                    player -> BehaviorCompat.matchesNonCombatRange(this, player, 4.0D));
        }
