...
$treeMixin = Get-Content -LiteralPath (Join-Path $root 'src\nibaru\java\games\twinhead\moreslabsstairsandwalls\mixin\TreeDecoratorGeneratorMixin.java') -Raw
$alterMixin = Get-Content -LiteralPath (Join-Path $root 'src\nibaru\java\games\twinhead\moreslabsstairsandwalls\mixin\AlterGroundDecoratorMixin.java') -Raw
$conversion = Get-Content -LiteralPath (Join-Path $root 'src\nibaru\java\games\twinhead\moreslabsstairsandwalls\block\spreadable\PodzolGeometryConversion.java') -Raw
$client = Get-Content -LiteralPath (Join-Path $root 'src\main\java\dev\aero\cnmterraincompat\CnmTerrainCompatClient.java') -Raw
...
foreach ($model in @('template_grass_slab.json', 'template_grass_stairs.json', 'template_grass_wall_post.json',
    ...
    $path = Join-Path $root "src\nibaru\resources\assets\more_slabs_stairs_and_walls\models\block\$model"
    if ((Get-Content -LiteralPath $path -Raw) -notmatch '"tintindex"\s*:\s*0') {
        throw "Representative tinted model lacks tintindex 0: $model"
    }
}

$generator = Get-Content -LiteralPath (Join-Path $root 'tools\nibaru\Generate-CurrentResources.ps1') -Raw
