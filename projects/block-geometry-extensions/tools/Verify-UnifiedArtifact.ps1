param(
    [string]$UnifiedJar = (Join-Path $PSScriptRoot '..\build\libs\BGE C88.jar'),
...
Require ([System.IO.Path]::GetFileName($unifiedPath) -ceq 'BGE C88.jar') `
        'Distributable artifact filename is not exactly BGE C88.jar'
...
try {
    ...
    Require ($cnmClasses.Count -eq 0) "C88 must retain stock CNM as an external dependency: $($cnmClasses -join ', ')"
    ...
    Require ($metadata.version -eq '4.2.32-bge.canary88.bbb-beam-model-resolution+26.2') `
            'Unified Fabric version is not exact C88'
    Require ($metadata.name -eq ('Block Geometry Extensions Canary 88 ' + [char]0x2014 + ' BBB Beam Model Resolution')) `
            'Unified Fabric display name is not exact C88'
