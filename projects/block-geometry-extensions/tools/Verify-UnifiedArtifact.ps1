param(
    [string]$UnifiedJar = (Join-Path $PSScriptRoot '..\build\libs\BGE C89.jar'),
...
Require ([System.IO.Path]::GetFileName($unifiedPath) -ceq 'BGE C89.jar') `
        'Distributable artifact filename is not exactly BGE C89.jar'
...
try {
    ...
    Require ($cnmClasses.Count -eq 0) "C89 must retain stock CNM as an external dependency: $($cnmClasses -join ', ')"
    ...
    Require ($metadata.version -eq '4.2.33-bge.canary89.cnm-family-root-classification+26.2') `
            'Unified Fabric version is not exact C89'
    Require ($metadata.name -eq ('Block Geometry Extensions Canary 89 ' + [char]0x2014 + ' CNM Family Root Classification')) `
            'Unified Fabric display name is not exact C89'
