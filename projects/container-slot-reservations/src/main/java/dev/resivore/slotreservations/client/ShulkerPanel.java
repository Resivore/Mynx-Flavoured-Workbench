...
public final class ShulkerPanel {
    ...
    /** Prevents the native-slot gesture from treating an overlaid panel cell as its underlay. */
    public static boolean containsPanel(double mouseX, double mouseY) {
        return binding != null && geometry != null && geometry.bounds().contains(mouseX, mouseY);
    }
    ...
    private static boolean sendContent(int slot, ShulkerPanelContentActionPayload.Click click) {
        if (binding == null || !ClientPlayNetworking.canSend(ShulkerPanelContentActionPayload.TYPE)) return false;
