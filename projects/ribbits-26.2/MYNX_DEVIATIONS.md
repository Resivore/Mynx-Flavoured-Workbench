...
The direct predecessor is Mynx Canary 3: version `4.1.6+26.2-mynx-canary3`, artifact `ribbits-private-reconstruction-4.1.6+26.2-mynx-canary3.jar`, 3,204,738 bytes, SHA-256 `69AD105B44E3E9131D540E26887EB09CE6BE1DA89EE9C4B3C49E4E95341F3E19`, source checkpoint `fc983c0534ac2a73308111d2052530a202156777`. Canary 4 preserves Canary 3 exactly and does not rebuild, overwrite, rename, promote, or misrepresent it. Canary 2 also remains historical and untouched at 3,166,148 bytes, SHA-256 `587B200A52300FF57C919ED5656E357DE5840D0D05C321D6D055405382D299BD`, source checkpoint `f0be1f9c6c1f3e843a0e44791a650df237836537`.

The current Phase C successor is `4.1.6+26.2-mynx-canary4`: private artifact `ribbits-private-reconstruction-4.1.6+26.2-mynx-canary4.jar`, 3,219,246 bytes, SHA-256 `DA7A37E0AEECE4D7633F0C5104298D76E066CD38DD5255FAE616DF95C5FBC712`; source-only artifact `ribbits-source-only-4.1.6+26.2-mynx-canary4.jar`, 1,159,347 bytes, SHA-256 `FE9E03AA72964CEB3EB3851EF485BAD50444AED00F6B8F1924DFFD0287CE01E4`. Its implementation checkpoint is assigned only after this content and the reproducible artifacts are frozen.
...
`SwampHutPiece.postProcess(WorldGenLevel, StructureManager, ChunkGenerator, RandomSource, BoundingBox, ChunkPos, BlockPos)` has four fail-loud `require=1` hooks: ordinal-0 `@ModifyArgs` on exact `getWorldPos(III)BlockPos$MutableBlockPos` for the resident, ordinal-0 `@Redirect` on exact `EntityType.create(Level,EntitySpawnReason)Entity`, `@Inject` immediately before ordinal-0 exact `spawnCat(ServerLevelAccessor,BoundingBox)V` for the barrel, and the Cat method's ordinal-0 `@ModifyArgs` on exact `getWorldPos`. `NaturalSpawner.isValidSpawnPostitionForType(ServerLevel,MobCategory,StructureManager,ChunkGenerator,MobSpawnSettings$SpawnerData,BlockPos$MutableBlockPos,double)boolean` has one cancellable head injection with `require=1`. Static ASM tests require every declaration and targeted call count and their relevant ordering exactly.
