...
## Canary 27 small-brown presentation and Gardener tier-1 selection

- The immutable Matcha archive remains 2,616,259 bytes / SHA-256 `2642DCEA338100F469DF905B212423683E83AE7C683C7B9FABFBD2195A2FE802`; its approved surface now also includes `assets/ribbits/textures/block/toadstool.png`, 2,780 bytes / SHA-256 `CFCB0792DCF4A5C54512133FEB3B45B88332F58ECA6B68910ACC7040E04CAE0A`, 64×64 RGBA. The independently supplied immutable `originals/assets/brown toadstool.png`, 348 bytes / SHA-256 `4121BC35ABFEB82E2A29D14D07A8C422CA9372BDCCE9841BE5BD1807CD696F7E`, is 14×14 RGBA. Assembly centers it on a transparent 16×16 canvas and converts only exact blue `(0,162,232,255)` key pixels to transparency; it never bundles either donor.
- The small-brown placed atlas derives only from the approved 64×64 Matcha atlas and existing approved brown palette: all 797 red/spot cap pixels become the exact brown palette while its 515 stem pixels remain unchanged. The generated block atlas is 1,120 bytes / SHA-256 `FBDB8FBB33F55A22D0E98587E07FF070442B3A36C30D61A159EB0DB7B81CD9E3`; the centered item is 238 bytes / SHA-256 `252024E0AE361F87004FF1D877596B16C5E0F6062081102C242EDBB29FE9C9B6`. Canary 26's four volumetric models, huge-growth contracts, and future-only village variation remain unchanged.
- Gardener tier 1 replaces the former two-of-four paired branch with a persistent, independently materialized three-of-five selection: Red Mushroom → Toadstool, Brown Mushroom → Brown Toadstool, Oxeye Daisy → Swamp Daisy, Lily Pad → Giant Lilypad, and Small Dripleaf → Umbrella Leaf. The native trade schema is 2; legacy saved pair `0` maps to mask `00101` and pair `1` maps to `11000`, so existing choices stay deterministic rather than rerolling. No recipe, Toadstool Heart, Wandering Ribbit, ordinary worldgen, or existing-village retrofit is added.

Two independent C27 private assemblies are byte-identical at 363 files / 266 strict JSON documents / 2,780,713 bytes. The private C27 archive is 3,377,508 bytes / SHA-256 `088C4B7E88C432D6275395E29273597CF42575F59350FE8B4F48C46E6DF7F9FD`; this is static evidence only and C27 remains `RUNTIME_UNTESTED`.
...
- Optional provider `ribbits:optional_naturalist_fauna`, schema 1, independently selects two available approved fauna results. Each exact persisted fauna range member has `maxUses=1`, zero XP, zero price multiplier, and `NEVER_RESTOCK`. Because vanilla `MerchantOffer.take` validates payment but not out-of-stock state, the exact Naturalist range is restored as a guarded one-shot offer after codec load; a stale completion after exhaustion returns false before consuming payment. The guard does not wrap native or Matcha-compass offers, does not reroll saved selections, and ordinary restocks reset only ordinary provider ranges.
...
The cumulative thresholds are Tier 1 at 0 XP, Tier 2 at 5, Tier 3 at 15, Tier 4 at 30, and Tier 5 at 50. Ordinary Tier 1–4 offers award 1, 2, 3, and 4 profession XP; a profession's maximum-tier offers award zero. Lower-tier offers remain and retain their tier XP until maximum rank. New tiers materialize immediately. Every designed barter has zero price multiplier. Tiered professions show progress; Musician remains unranked with no bar.

Saved progression uses those exact thresholds. Migration clamps only invalid rank bounds, reconciles a saved rank upward when saved XP now reaches a lower threshold, materializes only the newly unlocked current-schema tiers, and never lowers saved XP or rebuilds existing saved offers. Benzene still caps Sorcerer below rank 2 until traded and floors it at 5 XP afterward; Opal still caps Fisherman below rank 5 until traded and floors it at 50 XP afterward. A display-only adapter proportionally maps progress to Minecraft's fixed merchant-bar breakpoints and keeps five-tier professions on the fourth visible segment at maximum so the vanilla screen renders a full bar rather than suppressing it at vanilla level 5; it does not change rank, earned XP, or persistence.
