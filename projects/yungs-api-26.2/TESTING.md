...
7. Run the paired Ribbits village, registry, save/reload, restart, and broader smoke checks from that exact Ribbits candidate's current `TESTING.md`, recording YUNG-relevant observations separately. No canonical Onion Town procedure or identifier currently exists in this repository, so do not invent an Onion Town result.
