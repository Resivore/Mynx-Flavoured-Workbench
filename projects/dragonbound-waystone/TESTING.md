...
This migration performed no Minecraft runtime test. The current accepted evidence remains the user's focused confirmation that exact `dragonbound-waystone-0.1.0-canary5.jar`, SHA-256 `FF29582CC50F391A3EBB79B5AB1DFF2F8FB7ABB92363BCA46FB8A9608A481AD3`, looks good and that Dragonbound is ready to move on from. No Canary 5 `latest.log` exists, so do not infer that each step below was individually re-observed on Canary 5.
...
2. Verify the exact artifact filename, embedded `dragonbound_waystone` / `0.1.0-canary5` identity, 60,263-byte size, and SHA-256 above before deployment.
...
3. Main-hand click the Imbued Void once. Confirm the default channel is approximately 40 server ticks, releasing the mouse does not cancel it, exact arrival is at block center and `blockY + 3/16`, and one Pearl is consumed only after confirmed arrival.
4. Confirm movement and accepted damage cancel the pending channel. Confirm death, disconnect, dimension change, held-slot or component change, relocation, unsafe state, and an obstructed exact destination fail cleanly without consumption or nearby fallback.
...
6. Use the Dragonbound Staff once. Confirm the accepted transparent 16 x 16 sprite, the same return rules, success-only 1,200-tick cooldown, and no durability, charge, fuel, or Pearl consumption.
