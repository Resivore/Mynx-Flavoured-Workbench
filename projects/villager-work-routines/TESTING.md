...
Candidate `villager-work-routines-0.1.0-canary28.jar` / SHA-256 `3526e6495c22c64998f0d7ccf8bcaeaeb76dc01ddc6f4ba6d2d61a291d454767` / source `d82d1eb6394c3254b09b50c77bfeb29e3fc8669e` is a user-managed Minecraft Java 26.2/Fabric trial. Verify that identity against `WORKBENCH_STATUS.json`, together with exact retained Ribbits Canary 27 `ribbits-private-reconstruction-4.1.6+26.2-mynx-canary27.jar` / SHA-256 `088c4b7e88c432d6275395e29273597cf42575f59350fe8b4f48c46e6df7f9fd`. Use the normal Frog Villager resource pack. Build and static validation are not runtime evidence. Do not install the Real Working Villagers reference JAR or access a protected Minecraft profile.

## Canary 28 Fisherman rod micro-alignment and routine regression trial

### Historical evidence boundary

The supplied C27 screenshot is recorded as **C27 runtime FAIL for rod placement only**: the `-2px` folded-arm-local Z experiment moved the rod substantially outward/away from the Frog Villager's hands. The proven `+2px` local-Y/down correction remains unchanged. C28 changes only final local Z to `+3px`, giving final `(X 0px, Y +2px, Z +3px)` relative to the resolved C24 grip: `+2px` inward from C25/C26 and `+5px` opposite the visibly bad C27 placement. Do not infer any other Fisherman, productivity, deposit, Shepherd, or general runtime result. C28 is **ACTIVE / STATIC_PASS / RUNTIME_UNTESTED** until this owner-managed trial occurs.
...
1. Start exact VWR C28 and Ribbits C27 with Frog Villager resources enabled. There must be no VWR mixin, GeoLib, EMF, renderer, model/resource-resolution, or authored-path failure.
...
3. Verify the rod retains the good C25/C26/C27 downward placement, moves substantially inward from C27's visibly bad outward position, and sits closer to the Frog Villager's folded hands. It must still use the semantic EMF `arms -> arms_rotation` attachment, authored `[0,-7,-6]px` reference / `invertAxis="xy"` mapping, and neutralized Ribbits C27 baked rotation: no added rotation or scale is valid.
...
5. Verify the green-star physical marker remains at the visible `-9.5px` outer shaft tip and VWR's sole 16-segment line starts exactly at that captured point. `C28_line_start_minus_visible_tip_camera_relative` must be zero or floating-point-equivalent zero. Retain concise `[VWR C28 ROD DIAGNOSTIC: ...]` blocks.
