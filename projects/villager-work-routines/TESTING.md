...
Candidate `villager-work-routines-0.1.0-canary32.jar` / SHA-256 `4fd99bd8c17b676585ab4945cd9167c38301cf66f88d0bbf2b42911c0f394298` / source `ad61618988ff0c3f3711bf90746f713cbbb14f6e` is a user-managed Minecraft Java 26.2/Fabric trial. Verify its finalized identity against `WORKBENCH_STATUS.json`, together with exact retained Ribbits Canary 27 `ribbits-private-reconstruction-4.1.6+26.2-mynx-canary27.jar` / SHA-256 `088c4b7e88c432d6275395e29273597cf42575f59350fe8b4f48c46e6df7f9fd`. Use the normal Frog Villager resource pack. Build and static validation are not runtime evidence. Do not install the Real Working Villagers reference JAR or access a protected Minecraft profile.

## Canary 32 Fisherman shaft-axis rod translation and routine regression trial

### Historical evidence boundary

The supplied C31 screenshot is **C31 Fisherman rod-placement runtime evidence only**: the grip is an acceptable baseline, but the desired remaining movement is two model pixels parallel to the physical shaft from the grip toward its outer tip. It establishes no Fisherman productivity/deposit, Shepherd, or unrelated runtime result. C31's exact retained post-C24 correction is C30 `(X 0px, Y +1px, Z +7px)` plus its live folded-arm-basis-derived parent-space `(0,-2,0)px` delta; in the normal inspected 43-degree basis that extra local delta is `(ΔX 0, ΔY -1.462707, ΔZ +1.363997)px`, yielding C31 `(X 0, Y -0.462707, Z +8.363997)px` relative to C24. C32 preserves that exact C31 pose for the translucent ghost. It translates only the solid complete rod by the normalized authoritative grip-to-physical-tip vector `(0,0,-9.5px)` times `2px`: rod-local `(ΔX 0, ΔY 0, ΔZ -2px)`, or `(0,0,-0.125)` model/block units. C32 is **ACTIVE / STATIC_PASS / RUNTIME_UNTESTED** until this owner-managed trial occurs.
...
1. Start exact VWR C32 and Ribbits C27 with Frog Villager resources enabled. There must be no VWR mixin, GeoLib, EMF, renderer, model/resource-resolution, or authored-path failure.
...
3. Verify the translucent violet C31 ghost is the exact unmodified C31 pose and the solid C32 rod is exactly two model pixels farther from the grip toward the physical outer shaft tip, parallel to the existing shaft/red-arrow axis. The hands should intercept the existing rod length farther up its shaft. Orientation and apparent rod length must remain unchanged: no extension, deformation, rotation, or scale is valid. It must still use the semantic EMF `arms -> arms_rotation` attachment, authored `[0,-7,-6]px` reference / `invertAxis="xy"` mapping, and neutralized Ribbits C27 baked rotation.
4. Verify one solid C32 rod, its translucent C31 comparison ghost, and one real line are visible. The ghost has no fishing line and receives zero C32 shaft translation. There must be no colored diagnostic axes, crosses, stars, squares, diamonds, grip markers, transform rays, or comparison-marker clutter; `fishing_rod_2` / `fishing_rod_3` decoration, vanilla fishing equipment, a second line, or a second bobber are invalid.
5. Verify the real VWR 16-segment line begins at the translated visible physical `-9.5px` outer shaft tip of the solid rod. The translated physical tip remains its sole authority; do not expect diagnostic markers or per-cast transform diagnostics in the normal release.
