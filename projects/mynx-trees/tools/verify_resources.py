...
class Resources(unittest.TestCase):
    ...
    def test_silver_birch_tint_and_texture_contracts_stay_exact(self):
        ...
        self.assertEqual('h = sin(x/10 + sin((z+x)/50)*3)/75 + 0.15; s = 0.8; v = 0.52; float arithmetic, Minecraft sine lookup, RGB rounding as Java HSBtoRGB; v is the current intentional render compensation, not byte-for-byte RU raw provider output',color['leaf_hsv'])
        self.assertEqual('#857A1B',color['inventory_rgb']);self.assertEqual('#B0C73A',color['grass'])
