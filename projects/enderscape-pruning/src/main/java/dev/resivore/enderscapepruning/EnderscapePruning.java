...
/** Registers C1's always-enabled, narrowly generated Enderscape data overlay. */
public final class EnderscapePruning implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        boolean registered = ResourceLoader.registerBuiltinPack(
                BUILTIN_PACK_ID,
                container,
                Component.literal("Enderscape Pruning C1"),
                PackActivationType.ALWAYS_ENABLED);
        if (!registered) {
            throw new IllegalStateException("Could not register required Enderscape Pruning data overlay");
        }
        LOGGER.info("Registered Enderscape Pruning C1 overlay; exact upstream input SHA-256={}",
