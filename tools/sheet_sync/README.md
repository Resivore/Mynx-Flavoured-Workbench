...
Live sending is owned exclusively by the GitHub Actions `Reconcile Project Status Sheet` workflow after a push to authoritative `main`. Its `sheet-production` environment supplies `MYNX_SHEET_CUTOVER=authorized`, the receiver URL, and the HMAC secret; development, Codex, and Work hosts must neither contain nor inspect those values. Local use is limited to secretless planning and validation. The workflow requires tracked `publication.json` to be enabled, its environment to permit the main branch, and all production gates to pass. The Apps Script receiver adds its own default-off `MYNX_STATUS_ACCEPT_WRITES` Script Property and exact source checks. Feature branches cannot prepare an authoritative plan.

After a project revision is integrated and pushed to `main`, inspect that workflow run when GitHub Actions access permits. A successful `Publish independently valid project rows` step is a successful Sheet publication; a running run is pending verification; a failed or skipped run must be reported with its GitHub Actions reason. Only an inability to inspect GitHub Actions makes publication unverifiable. A missing local cutover variable, receiver URL, or HMAC secret is expected and must never be reported as a Sheet failure or project blocker.
...
The receiver serializes writes, flushes canonical fields first, writes `Event ID`, and commits `Revision` last. The HTTP publisher allows 60 seconds for a receiver response, while the receiver waits at most 5 seconds for its mutation lock. Ordinary contention can therefore return a structured `busy` response with substantial transport headroom; publishers retry only that explicit response with the same signed event. A generic transport timeout remains ambiguous and is not retried automatically. Stale and conflicting writes are permanent failures. CLI publication logs identify every event by Event ID, project UUID, and revision, including the receiver's boolean `changed` value on success, without exposing secrets.

Project sorting is presentation-only and is not part of the per-event mutation or acknowledgement path. After a publication or reconciliation batch, an authorized Apps Script operator may run `sortProjectsNow` to restore lifecycle/activity ordering. That function uses the same script lock and sorts the complete table width, including `Notes`, so human-owned values remain attached to their UUID row. A sorting failure cannot undo a row whose final `Revision` marker has already been flushed.
...
After deploying the repository-backed receiver, dispatch `Publish Project Status` with `bootstrap-current-state` from current authoritative `main`. Completion requires a successful structured acknowledgement for every participating UUID at its current canonical revision and preservation of all human-owned `Notes` cells. Do not seed historical events or edit Sheet/project revisions to make reconciliation pass.
