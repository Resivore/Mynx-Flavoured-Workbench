...
Real-server implementation state is a separate human-owned concept stored by immutable project UUID in `WORKBENCH_SERVER_STATE.json`. Missing entries mean `UNKNOWN`, never `NO`. Change a value only from explicit owner direction with `python tools/workbench_dashboard.py server <project-id-or-uuid> yes`, using `no` or `unknown` as appropriate; the command also regenerates the dashboard. Project IDs, names, and aliases are lookup conveniences only—the durable key remains the UUID. This record is independent of Workbench validation/deployment evidence and Google Sheet synchronization metadata.
