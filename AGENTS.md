...
For real project work, advance the manifest revision exactly once, update status metadata, append one log entry, and preserve exact Canary/version, filename, SHA-256, and source-checkpoint identity. New current artifacts need a concise one-sentence release summary for the dashboard. `source_commit` is the preceding coherent implementation/artifact checkpoint, never the commit containing its own hash.
...
- No external project-status publication, reconciliation, recovery, credentials, or completion condition exists in this workflow.
