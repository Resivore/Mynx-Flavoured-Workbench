...
For real project work, advance the manifest revision exactly once, update status metadata, append one log entry, and preserve exact Canary/version, filename, SHA-256, finalized `built_at` timestamp, and source-checkpoint identity. Every non-null current release carries an explicit positive integer `canary`: unchanged artifact bytes retain their ordinal even when release metadata is corrected, while genuinely new current bytes advance exactly once from the previous current Canary. Lifecycle, acceptance, administrative, evidence, and manifest-revision-only changes never advance it. Accepted and rollback records retain the ordinal established when those exact releases were current; do not invent an ordinal for imported historical releases where canonical evidence establishes none. The Canary is a human-facing Workbench identity independent of internal versions, filenames, hashes, `dev.N`, semantic versions, and `synchronization.revision`, and must never be derived from them. Integration validation permits at most one manifest transition per project in one push or pull-request range; publish sequential transitions for the same project in separate ranges. New current artifacts need a concise one-sentence release summary for the dashboard. Record `built_at` from the exact finalized, SHA-256-verified bytes, never a task-start approximation. `source_commit` is the preceding coherent implementation/artifact checkpoint, never the commit containing its own hash.
